...
# Export the 3-year time series line graph in 600 dpi
ggsave("3_Year_Monthly_Rainfall_Time_Series.png", width = 16, height = 9, units = "in", dpi = 600)
...
ggsave("Monthly_Rainfall_Smoothed_Line_Plot_2022-2024.png", width = 16, height = 9, units = "in", dpi = 600)

## WATER QUALITY DATA VISUALISATION ##
# Create a Boxplot for each location for each measure for water quality data
...
# Export the Boxplot for each location for each measure for water quality data
ggsave("Monthly_Avg_WQ_Boxplot.png", dpi = 600)
...
ggplot(water_quality_data, aes(x = as.Date(paste(year, month, "01", sep = "-")), y = monthly_avg_value, color = location_name, group = location_name)) +
  ...
  theme_minimal() +
  theme(legend.title = element_blank(), legend.text = element_text(size = 12), aspect.ratio = 9/16, panel.background = element_rect(fill = "lightblue"), text = element_text(size = 24), axis.text.y = element_text(size = 12)) +
  scale_x_date(date_breaks = "6 months", date_labels = "%b %Y") +
  theme(panel.grid.major.x = element_line(color = "grey80", linetype = "dashed"), axis.text.x = element_text(angle = 45, hjust = 1, size = 12))
